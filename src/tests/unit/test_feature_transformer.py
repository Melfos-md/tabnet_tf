...
def test_feature_transformer_batch_size_check():
    ...
    model = FeatureTransformer(N_a=N_a, N_d=N_d, shared=True, virtual_batch_size=virtual_batch_size)
    ...
    with pytest.raises(ValueError, match=r"Input batch size .* should be greater than or equal to the virtual batch size .*"):
        model(input_tensor)
